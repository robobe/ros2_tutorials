...
class TurtleGoToNode(Node):
    def __init__(self):
        ...
        self.__subscription_pose_request = self.create_subscription(PoseStamped, RVIZ_TOPIC, self.__rviz_pose_handler, 10)
        self.__subscription_pose_request
        self.__pose = Pose()
        self.__timer = self.create_timer(PERIOD, self.__timer_callback)
        self.__timer
        self.__tf_broadcaster = TransformBroadcaster(self)
        self.__goto_point = GOTO_TEST
    ...
    def __rviz_pose_handler(self, msg: PoseStamped):
        self.get_logger().info("Request new position {}, {}".format(msg.pose.position.x, msg.pose.position.y))
        x, y = translate(msg.pose.position.x, msg.pose.position.y, 5, 5)
        self.get_logger().info("New position after translate {}, {}".format(x, y))
        #self.__goto_point = Point(msg.pose.position.x, msg.pose.position.y)
    ...
    def __timer_callback(self):
        ...
        send twist
        """
        x_distance = LINEAR_KP * self.__calc_euclidean_distance(self.__goto_point)
        z_angle = ANGLE_KP * self.__calc_angle(self.__goto_point)
